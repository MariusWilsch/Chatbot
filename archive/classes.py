...
class ConfidenceDetails(BaseModel):
    ...
    missing_to_increase_confidence: List[str] = Field(
        default=[],
...
class ConfidenceDates(BaseModel):
    ...
    missing_to_increase_confidence: List[str] = Field(
        default=[],
        description="A list of missing key details needed to confidently extract all the required dates and determine the status of the legal case. If no missing details are present, this field will be an empty list.",
    )
    confidence: bool = Field(
        default=False,
